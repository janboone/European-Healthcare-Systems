...
#+caption: Relation between log treatable mortality (by gender) and log GDP per head.
#+name: fig:GDP_mortality
[[file:./figures/gdp_mortality_log_log_scale_by_sex.png]]
...
#+caption: Variation in nurses/doctors ratio across countries and time.
#+name: fig:nurses_doctors
[[./figures/nurses_doctors.png]]
...
#+begin_src jupyter-python :file ./figures/nurses_doctors.png
import matplotlib.colors as colors
import matplotlib as mpl
import matplotlib.cm as mplcm
import numpy as np
...
cm = plt.get_cmap('tab20b') # plt.get_cmap('gist_rainbow')
cNorm  = colors.Normalize(vmin=0, vmax=NUM_COLORS-1)
scalarMap = mplcm.ScalarMappable(norm=cNorm, cmap=cm)

#selection = (nurses_doctor_ratio>0)#*(log_gdp<0)
mpl.rcParams['axes.prop_cycle'] = mpl.cycler(color=([scalarMap.to_rgba(i) for i in range(NUM_COLORS)]))

for country in df.country.unique():
    mask = (df.country == country)
    plt.scatter(df[mask]['Practising medical doctors per 100k'], df[mask]['Practising nurses and midwives per 100k'], label = country)
    
plt.xlabel('Medical doctors per 100k')
plt.ylabel('Nurses and midwives per 100k')
plt.plot([0,550],[0,550],'--',c='k')
plt.plot([0,550],[0,5*550],'--',c='k')
plt.text(400,300, 'ratio 1')
plt.text(200,1300, 'ratio 5')
plt.xlim(0,550)
plt.ylim(0,1900)
plt.tight_layout()
#plt.title('Treatable mortality vs. GDP per head on log-log scales')
plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left', borderaxespad=0.);
#+end_src

#+RESULTS:
[[file:./figures/nurses_doctors.png]]
...
#+caption: The relation between median age in the population and a standardized mortality measure.
#+name: fig:age_mortality
[[file:./figures/age.png]]
...
#+begin_src jupyter-python :file ./figures/age.png
mask_f = df.sex == 'F'
plt.scatter(df['median age'],df['Treatable mortality'],label='men')
plt.scatter(df[mask_f]['median age'],df[mask_f]['Treatable mortality'],label='women')
plt.xlabel('age (in years)')
plt.ylabel('mortality per 100,000')
plt.legend()
plt.title('Treatable mortality vs. median age in the population by gender');
#+end_src
...
#+caption:  Model fit in terms of log treatable mortality and log GDP per head.
#+name: fig:fit_model
[[file:./figures/check_fit.png]]

# https://bookdown.org/rdpeng/advstatcomp/
# book on advanced statistical computing
# section 7.4: mcse vs r-statistic
...
#+begin_src jupyter-python :file ./figures/check_fit.png
mask_female = (df.sex=='F')
mask_male = (df.sex=='M')
import matplotlib.colors as colors
import matplotlib as mpl
import matplotlib.cm as mplcm
...
# ax2.vlines(log_gdp[mask_male],percentiles[0,:][mask_male],percentiles[1,:][mask_male],color='grey',alpha=0.4)

for country in df.country.unique():
    mask = (df.country == country)
    ax1.vlines(log_gdp[mask_female&mask],percentiles[0,:][mask_female&mask],percentiles[1,:][mask_female&mask], color = next(ax1._get_lines.prop_cycler)['color'],alpha=0.3)
    ax1.scatter(log_gdp[mask&(df.sex=='F')], log_mortality[mask&(df.sex=='F')], label = country)
    ax2.vlines(log_gdp[mask_male&mask],percentiles[0,:][mask_male&mask],percentiles[1,:][mask_male&mask], color = next(ax2._get_lines.prop_cycler)['color'],alpha=0.3)
    ax2.scatter(log_gdp[mask&(df.sex=='M')], log_mortality[mask&(df.sex=='M')], label = country)
...
#+begin_src jupyter-python :display plain
data_posterior_normal_extended = az.from_netcdf('./samples_posterior/trace_normal_extended.nc')
#+end_src

#+begin_src jupyter-python
single_private = data_posterior_normal_extended.posterior.b_single
percentiles_single_private = np.percentile(single_private,[2.5,97.5],axis=[0,1])

primary_care_multiple_public = data_posterior_normal_extended.posterior.b_public_primary_care
percentiles_primary_care_multiple_public = np.percentile(primary_care_multiple_public,[2.5,97.5],axis=[0,1])

primary_care_single_public = single_private + primary_care_multiple_public + data_posterior_normal_extended.posterior.b_single_public_primary_care
percentiles_primary_care_single_public = np.percentile(primary_care_single_public,[2.5,97.5],axis=[0,1])


inpatient_care_multiple_public = data_posterior_normal_extended.posterior.b_inpatient_public
percentiles_inpatient_care_multiple_public = np.percentile(inpatient_care_multiple_public,[2.5,97.5],axis=[0,1])

inpatient_care_single_public = single_private + inpatient_care_multiple_public + data_posterior_normal_extended.posterior.b_single_inpatient_public
percentiles_inpatient_care_single_public = np.percentile(inpatient_care_single_public,[2.5,97.5],axis=[0,1])


outpatient_care_multiple_public = data_posterior_normal_extended.posterior.b_outpatient_public
percentiles_outpatient_care_multiple_public = np.percentile(outpatient_care_multiple_public,[2.5,97.5],axis=[0,1])

outpatient_care_single_public = single_private + outpatient_care_multiple_public + data_posterior_normal_extended.posterior.b_single_outpatient_public
...
variables_expenditure = ['b_hc', 'b_hc1','b_hc2','b_hc3','b_hc4','b_hc5','b_hc6','b_hc7'] 
variables_health = ['b_female','b_gdp', 'b_female_gdp', 'b_inequality', 'b_smoking', 'b_obese', 'b_alcohol']
# variables_demand = ['b_social'] #'b_unmet',
variables_efficiency = ['b_nurses_doctors', 'b_avoidable_hospitalizations', 'b_ce_analysis'] #,'b_gatekeeper'
variables_market = ['b_hospital_choice', 'b_single','b_public_primary_care','b_inpatient_public','b_outpatient_public'] #  'b_pfp_GP', 'b_capacity_regulation','b_budget','b_primary_care_gatekeeping',
variables_quality = ['b_healthcare_regulator',  'b_quality_reports'] # 'b_continuous_education',
variables_interaction = ['b_reports_no_choice'] #, 'b_single_inpatient_public', 'b_single_outpatient_public', 'b_single_public_primary_care'] # 'b_reports_choice',
...
az.plot_trace(data_posterior_normal.posterior,var_names=variables_expenditure);
...
az.style.use("arviz-darkgrid")
az.plot_trace(data_posterior_normal.posterior,var_names=variables_health);
...
az.style.use("arviz-darkgrid")
az.plot_trace(data_posterior_normal.posterior,var_names=variables_efficiency);
...
az.style.use("arviz-darkgrid")
az.plot_trace(data_posterior_normal.posterior,var_names=variables_market);
...
az.style.use("arviz-darkgrid")
az.plot_trace(data_posterior_normal.posterior,var_names=variables_quality);
...
az.style.use("arviz-darkgrid")
az.plot_trace(data_posterior_normal.posterior,var_names=variables_interaction);
