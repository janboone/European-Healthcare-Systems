...
With this document, any reader can retrace the code which we used to produce the exact results, figures, tables, graphs, etc. for this paper. 

This file is written in [[https://www.gnu.org/software/emacs/][Emacs]] [[https://orgmode.org/][org mode]] which allows us to combine text and code. The file is exported to pdf (via latex) and to html for the web-version. The web-version --which you are reading now-- contains the sections tagged =code= which are left out of the pdf version of the paper. Here you can download the [[https://github.com/janboone/European-Healthcare-Systems/raw/main/EuropeanHealthcareSystems.pdf][pdf version of the paper]].
...
#+caption: Countries with the optimal binary choices
...
#+begin_src jupyter-python :display plain
groupby_policy.columns
#+end_src

#+RESULTS:
#+begin_example
Index(['Unnamed: 0', 'year', 'Preventable mortality', 'Treatable mortality',
       'potential years of life lost', 'median age', 'Bad Very Bad',
       'Very Good Good', 'high bmi', 'smoking', 'binge drinking',
       'Practising medical doctors per 100k', 'GDP per capita',
       'Practising nurses and midwives per 100k', 'avoidable hospitalizations',
       'HC1', 'HC2', 'HC3', 'HC4', 'HC5', 'HC6', 'HC7', 'year_y', 'y_QU1',
       'y_QU2', 'y_QU3', 'y_QU4', 'y_QU5', 'Year_system_date',
       'single payer system', 'inpatient public', 'outpatient public',
       'public primary care', 'free hospital choice',
       'almost free hospital choice', 'public provider quality reports',
       'healthcare regulator', 'ce analysis', 'hospital choice'],
      dtype='object')
#+end_example
...
- =avoidable hospitalizations= is taken from cite:oecdHospitalAdmissions. We use asthma and COPD hospital admissions in adults (per 100k population). Since primary care can deal with most of these cases, this variable captures gate keeping and the level of organization and cooperation at primary care level;
- =ce analysis=: this dummy variable is taken from the OECD Health Systems Characteristics Survey (https://qdd.oecd.org/data/HSC);  in particular, the variable equals 1 for countries that answer "yes" to the question "Do HTAs generally include results of economic evaluation? (Q62b)", where HTA stands for health technology assessment.
